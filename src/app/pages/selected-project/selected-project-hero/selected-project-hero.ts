...
import { ProjectHero } from '../../../models/project.model';
...
export class SelectedProjectHero implements AfterViewInit {
  @Input({ required: true }) hero!: ProjectHero;
  ...
  ngAfterViewInit(): void {
    ...
    const media = host.querySelector<HTMLElement>('.selected-project__media');
    ...
    if (media) tl.fromTo(media, { scale: 1.1, autoAlpha: 0 }, { scale: 1.04, autoAlpha: 1, duration: 1.1 }, 0);
    ...
    if (media) this.anim.parallaxLayer(media, { speed: -15 });
  }
}
